...
@Slf4j
public class SshWithLocalDbTest extends BaseIntegrationTestConfiguration {

    @Test
    public void testProofOfTestingForSshCommand() {
        String process = "SSH_BG";
        String bashCommand = "{ echo 123; }";
        ExecutionRequest request = createSshRequest(process, bashCommand,
                getSshTestParams(sshServerHost + ":" + sshPort, 99000, 2000));
        request.setSessionId(miaContext.getFlowData().getSessionId());
        // request build&run
        webClient.post().uri(uriBuilder ->
                ...
                .block();
        //save POT
        List<Link> saveResult = webClient.post().uri(uriBuilder ->
                        uriBuilder.path("/rest/pot/save")
                                .queryParam("projectId", projectId.get())
